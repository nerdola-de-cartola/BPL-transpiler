...
// Mine
variavel VARIABLES[MAX_VARIABLE];
...
int main(int argc, char **argv)
{
   ...
   F_SOURCE = fopen("test.bpl", "rt");
   // if(F_SOURCE == NULL) error("impossible to read file");

   F_OUTPUT = fopen("test.s", "wt");
...
void subq(int lastStackPos)
{
   if (lastStackPos % 16 == 0)
   {
      fprintf(F_OUTPUT, "subq $%d, %%rsp\n", lastStackPos);
   }
   else
   {
      if ((lastStackPos + 4) % 16 == 0)
      {
         fprintf(F_OUTPUT, "subq $%d, %%rsp\n", lastStackPos + 4);
      }
      else if ((lastStackPos + 8) % 16 == 0)
      {
         fprintf(F_OUTPUT, "subq $%d, %%rsp\n", lastStackPos + 8);
      }
      else if ((lastStackPos + 8) % 16 == 0)
...
// Parte de atribuição (Matheus), não é imprtante para mim agora
